...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/darnix1/izin/main/IP > /root/tmp
...
# https://raw.githubusercontent.com/darnix1/izin/main/IP 
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/darnix1/izin/main/IP | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
